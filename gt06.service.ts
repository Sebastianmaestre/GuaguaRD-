...
export class Gt06Service implements OnModuleInit, OnModuleDestroy {
   ...
   * Requiere el modelo PositionHistory (ver prisma/schema.gt06.prisma)
   * y resolver `companyId`/`unitId` a partir del imei (ver TODO abajo).
   */
  private async saveToPostgres(position: Position) {
    try {
      // TODO: resolver unitId/companyId reales a partir de position.imei
      // (ej. buscando la unidad dada de alta con ese IMEI). Se deja el
      // imei como referencia directa mientras tanto.
